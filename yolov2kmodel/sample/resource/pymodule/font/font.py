FONT_12_CHAR_WIDTH = 6
FONT_12_CHAR_HEIGHT = 12
FONT_12_CHAR_SIZE = 12
font_1206 = [
...
FONT_16_CHAR_WIDTH = 8
FONT_16_CHAR_HEIGHT = 16
FONT_16_CHAR_SIZE = 16
font_1608 = [
...
FONT_24_CHAR_WIDTH = 12
FONT_24_CHAR_HEIGHT = 24
FONT_24_CHAR_SIZE = 36
font_2412 = [
...
FONT_32_CHAR_WIDTH = 16
FONT_32_CHAR_HEIGHT = 32
FONT_32_CHAR_SIZE = 64
font_3216 = [
...
def img_draw_point(img, x, y, color):
    img.draw_rectangle(x, y, 1, 1, color, 0, True)

def img_draw_char(img, x, y, char, font, color):
    ch_offset = ord(char) - ord(' ')
    if font == 12:
    ...
    if (x + ch_width > img.width()) or (y + ch_height > img.height()):
        return

    width_index = 0
    height_index = 0
    for byte_index in range(ch_size):
        byte_code = ch_code[byte_index]
        for bit_index in range(8):
            if (byte_code & 0x80) != 0:
                img_draw_point(img, x + width_index, y + height_index, color)
...
def img_draw_string(img, x, y, width, height, string, font, color):
    if font == 12:
        ch_width = FONT_12_CHAR_WIDTH
        ch_height = FONT_12_CHAR_HEIGHT
    elif font == 16:
    ...
    for char in string:
        ...
        img_draw_char(img, x, y, char, font, color)
